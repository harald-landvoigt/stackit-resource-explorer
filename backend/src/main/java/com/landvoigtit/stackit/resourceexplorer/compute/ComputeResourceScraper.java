...
public class ComputeResourceScraper {
    ...
    private boolean scrapeProjectCompute(final Project project, final List<String> currentResourceIds) {
        ...
        if (accessIssueRegistry != null) {
            if (permissionDenied) {
                accessIssueRegistry.recordFailure(projectIdStr, projectName, StackitConstants.RESOURCE_TYPE_COMPUTE, permissionDeniedRegion, 403, permissionDeniedMsg);
            } else if (allRegionsSucceeded) {
                accessIssueRegistry.recordSuccess(projectIdStr, projectName, StackitConstants.RESOURCE_TYPE_COMPUTE, null);
            }
        }
